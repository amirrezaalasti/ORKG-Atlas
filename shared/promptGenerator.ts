...
> = {
  ...
  R186491: {
    ...
    queryExamples: `
### Example: List sample papers
Template R186491 is a NodeShape resource, NOT a class. Wrong: \`?paper a orkgr:R186491\`. Correct:
...
`,
    queryExamples: `
### Example: Top evaluation metrics (with human-readable labels)
\`\`\`sparql
# id: top_metrics
SELECT ?metric_label (COUNT(DISTINCT ?paper) AS ?count)
WHERE {
  ?paper orkgp:P31 ?contribution .
  ?contribution a orkgc:C121001 .
  ?contribution orkgp:HAS_EVALUATION ?evaluation .
  ?evaluation orkgp:P110006 ?evaluation_metric .
  ?evaluation_metric rdfs:label ?metric_label .
}
GROUP BY ?metric_label
ORDER BY DESC(?count)
\`\`\`
Never chart or table ?evaluation_metric URIs — only ?metric_label.
`,
    specificRules: `
...
**ORDER BY (ORKG Virtuoso — syntax errors if wrong):**
- ✅ \`ORDER BY ?year\` or \`ORDER BY ASC(?year)\` (ascending is default)
- ✅ \`ORDER BY DESC(?count)\` or \`ORDER BY ?year DESC(?count)\`
- ❌ \`ORDER BY ?year ASC\` — invalid, causes "syntax error at '}'"
- ❌ \`ORDER BY ?year ?count DESC\` — invalid; use \`ORDER BY ?year DESC(?count)\` instead
...
- **Time-based analysis**: Include \`?year\` via \`?paper orkgp:P29 ?year\`; for counts by year use \`GROUP BY ?year ?methodLabel\` and \`ORDER BY ?year DESC(?count)\` (never \`?year ?count DESC\`)
- **Method analysis**: Traverse hierarchy (e.g. \`?contribution orkgp:P56008 ?dc . ?dc orkgp:P1005 ?method . ?method orkgp:P94003 ?method_type . ?method_type rdfs:label ?methodLabel\`) — do not attach P1005 directly to \`?contribution\`
